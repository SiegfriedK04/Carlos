...
function IndexPage() {
  const [authProvider, setAuthProvider] = useState<"local" | "clerk">("local");
  const [authMode, setAuthMode] = useState<"signIn" | "signUp">("signIn");
  const { clerkEnabled, getToken, isLoaded, isSignedIn } = useSessionAuth();
      ...
      <div className="auth-landing">
        <section className="panel auth-hero-panel">
          ...
          <div className="feature-strip">
            ...
            <span>Login local con hash</span>
        ...
        <section className="panel auth-card-panel">
          <div className="auth-card-head">
            <div>
              ...
              <h3>Elige como quieres entrar</h3>
            </div>
            <p className="muted-text auth-card-copy">
              Puedes usar login local con hash o continuar con Clerk si las llaves estan configuradas.
            </p>
          </div>

          <div className="segmented-row auth-provider-switch">
            <button
              type="button"
              className={authProvider === "local" ? "cta-link" : "cta-link secondary"}
            ...
            <button
              type="button"
              className={authProvider === "clerk" ? "cta-link" : "cta-link secondary"}
          ...
          {authProvider === "local" || !clerkEnabled ? <LocalAuthPanel /> : null}

          {authProvider === "clerk" && clerkEnabled ? (
            <div className="auth-method-panel">
              <div className="auth-method-head">
                <div>
                  <p className="eyebrow">Clerk</p>
                  <h3>{authMode === "signIn" ? "Ingresar con Clerk" : "Crear cuenta con Clerk"}</h3>
                </div>
                <p className="muted-text auth-method-copy">Mantiene el proveedor externo como segunda via de acceso.</p>
              </div>
          ...
          {!clerkEnabled ? (
            <p className="muted-text auth-disabled-note">
              Clerk no esta visible porque falta la publishable key en el frontend. Ahora tambien acepta `CLERK_PUBLISHABLE_KEY` como respaldo.
            </p>
          ) : null}
        </section>
      </div>
  ...
  return (
    <section className="panel lobby-shell">
      <div className="section-head">
        ...
        <button data-testid="create-game-button" type="button" className="cta-link" disabled={createGameMutation.isPending} onClick={() => createGameMutation.mutate()}>
      ...
      <div className="lobby-summary-row">
        <div className="summary-card compact-summary" data-testid="lobby-user-card">
