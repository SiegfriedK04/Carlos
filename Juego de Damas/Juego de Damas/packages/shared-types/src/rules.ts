...
export function searchBestMoveAStar(boardState: BoardState, currentPlayer: PieceColor, depth = 3): AStarSearchResult {
  ...
  for (const move of legalMoves) {
    ...
    const score = evaluateBoard(applied.boardState, currentPlayer) + move.captures.length * 6;
    pushSorted(frontier, {
    ...
    rootScores.set(JSON.stringify(move), score);
  }
  ...
  while (frontier.length > 0) {
    ...
    const existing = rootScores.get(rootKey) ?? node.score;

    if (node.maximizing) {
      rootScores.set(rootKey, Math.max(existing, node.score));
    } else {
      rootScores.set(rootKey, Math.min(existing, node.score));
    }

    if (node.depth >= depth || node.boardState.winner) {
      continue;
    }
    ...
    for (const nextMove of nextMoves) {
      ...
      const heuristic = evaluateBoard(applied.boardState, currentPlayer) + nextMove.captures.length * 4;
      pushSorted(frontier, {
        boardState: applied.boardState,
  ...
  const rankedMoves = legalMoves
    .map((move) => ({ move, score: rootScores.get(JSON.stringify(move)) ?? Number.NEGATIVE_INFINITY }))
  ...
  return {
    ...
    score: best.score,
    exploredNodes,
    summary: `A* exploro ${exploredNodes} nodos a profundidad ${depth} y priorizo ${
